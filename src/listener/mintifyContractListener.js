    ...
    switch (event.topics[0]) {
      case EVENT_TYPE.OrderFulfilled: {
        ...
        if (data.sale_type === SALE_TYPE.SELL) {
          ...
          await sellOrderSaleProcessInMintify({data, strapi,nftData}).catch(e => console.error(e.message))
...
const checkIsValidSellOrderSaleAndGetData = async ({strapi, data}) => {
  console.log(`[MIN] ${data.sale_type} - 1. checkIsValidSellOrderSaleAndGetData Start data:\n`,data);
     /** 
         * Validations
  ...
  try {
    ...
    if (!nftData)  {
      console.log(`[MIN] ${data.sale_type} - There is no nft data - contract_address: ${data.contract_address} | token_id: ${data.token_id}`);
      return false
    }
    ...
    if (existedTradeLog) {
      console.log(`[MIN] ${data.sale_type} - existedTradeLog - ${existedTradeLog}`)
      return false
    }
...
const checkIsValidBuyOrderSaleAndGetData = async ({strapi, data}) => {
  console.log(`[MIN] ${data.sale_type} - 1. checkIsValidBuyOrderSaleAndGetData Start data:\n`, data);
...
const sellOrderSaleProcessInMintify = async ({data, strapi, nftData}) => {
  console.log(`[MIN] ${data.sale_type} - 2. sellOrderSaleProcessInElement Start`);
  /** 
  ...
  if (nftData.sell_order) {
    console.log(`[MIN] ${data.sale_type} (optional) delete sell order id - ${nftData.sell_order.id} `);
  ...
  console.log(`[MIN] ${data.sale_type} - 3. nft id: ${nftData.id} updated owner ${nftData.owner} -> ${data.to}`);
  ...
  console.log(`[MIN] ${data.sale_type} - 4. nft id: ${nftData.id} created SALE log id ${createdLog.id}`);

}

const buyOrderSaleProcessInElement = async ({data, strapi, nftData}) => {
  console.log(`[MIN] ${data.sale_type} - 2. buyOrderSaleProcessInElement Start`);
  /** 
   * Wen DB 에 존재하는 NFT 임이 가정입니다. (Validation 완료)
