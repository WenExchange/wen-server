...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async earlyAccessBridge(ctx) {
      {
        try {
          let { signature, address, message } = ctx.request.body.data;
          const recoveredAddress = ethers.utils.verifyMessage(
            message,
            signature
          );
          ...
          const earlyUser = await strapi.db
            .query("api::early-user.early-user")
            .findOne({
          ...
          if (user.early_user) {
            return (ctx.body = {
              success: false,
              message: "You have already migrated.",
            });
          }
          ...
          try {
            ...
            const staked = await ogpassStakingContract.stakedTokensByUser(
              user.address
            );
            is_og = staked.length > 0;

            if (!is_og) {
              const ogpassContract = new ethers.Contract(
                "0x64e38aa7515826bcc00cece38f57ca21b1495710",
                ERC721,
                jsonRpcProvider_cron
              );
          ...
          try {
            ...
            const blurPointContract = new ethers.Contract(
          ...
          // 초대코드
          if (
            earlyUser.total_invite_point &&
            !Number.isNaN(earlyUser.total_invite_point)
          ) {
            pre_token += Number(earlyUser.total_invite_point);
          }
          if (earlyUser.guests) {
            const guestBoost =
              earlyUser.guests >= 10 ? 10 * 15 : 15 * earlyUser.guests;
            boost += guestBoost;
          }

          // bridging
          if (
            earlyUser.bridging_point &&
            !Number.isNaN(earlyUser.bridging_point)
          ) {
          ...
          // community incentive
          if (
            earlyUser.community_incentive &&
            !Number.isNaN(earlyUser.community_incentive)
          ) {
            ...
            const airdropHistoryLog = await strapi.db
              ...
              .create({
                data: {
                  ...
                  type: AIRDROP_TYPE.EARLY_ACCESS,
                  timestamp: dayjs().unix(),
                  airdrop_point,
                },
              });
          }
          let total_airdrop_point =
            user.total_airdrop_point &&
            !Number.isNaN(user.total_airdrop_point)
              ? Number(user.total_airdrop_point) : 0
              total_airdrop_point = total_airdrop_point + airdrop_point
          ...
          user = await strapi.db
            ...
            .update({
              ...
              data: {
                ...
                total_airdrop_point,
                total_bridging_point: airdrop_point
              },
              populate: {
                early_user: true
              }
