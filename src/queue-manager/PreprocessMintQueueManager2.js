...
let instance = null;
module.exports = class PreprocessMintQueueManager2 {
  PROCESS_QUEUE = [];
  isProcessing = false
  ...
  static getInstance(strapi) {
    if (!instance) {
      instance = new PreprocessMintQueueManager2(strapi);
    }
    return instance;
  }
  ...
  processQueue = async () => {
    ...
    while (this.PROCESS_QUEUE.length > 0) {
      console.log(`[PreprocessMintQueueManager2] Processing Queue Start - ${this.PROCESS_QUEUE.length} -> ${this.PROCESS_QUEUE.length - 1}`);
  ...
  processSecondQueue = async () => {
    ...
    while (this.SECOND_PROCESS_QUEUE.length > 0) {
      console.log(`[PreprocessMintQueueManager2] Second Processing Queue Start - ${this.SECOND_PROCESS_QUEUE.length} -> ${this.SECOND_PROCESS_QUEUE.length - 1}`);
