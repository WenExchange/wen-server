...
const openseaContractListener = async ({ event, strapi }) => {
  try {
...
const checkIsValidSellOrderSaleAndGetData = async ({ strapi, data }) => {
  console.log(
    `[OPENSEA] ${data.sale_type} - 1. checkIsValidSellOrderSaleAndGetData Start data:\n`,
    data
  );
  /**
   * Validations
  ...
  try {
    ...
    if (!nftData) {
      console.log(
        `[OPENSEA] ${data.sale_type} - There is no nft data - contract_address: ${data.contract_address} | token_id: ${data.token_id}`
      );
      return false;
    }
    ...
    if (existedTradeLog) {
      console.log(
        `[OPENSEA] ${data.sale_type} - existedTradeLog - ${existedTradeLog}`
      );
      return false;
    }
    ...
    console.error(`[OPENSEA] ${data.sale_type} - error - ${error.message}`);
    return false;
  }
};
...
const checkIsValidBuyOrderSaleAndGetData = async ({ strapi, data }) => {
  console.log(
    `[OPENSEA] ${data.sale_type} - 1. checkIsValidBuyOrderSaleAndGetData Start data:\n`,
    data
  );
...
const sellOrderSaleProcessInMintify = async ({ data, strapi, nftData }) => {
  console.log(
    `[OPENSEA] ${data.sale_type} - 2. sellOrderSaleProcessInElement Start`
  );
  /**
  ...
  if (nftData.sell_order) {
    console.log(
      `[OPENSEA] ${data.sale_type} (optional) delete sell order id - ${nftData.sell_order.id} `
    );
  ...
  await strapi.entityService
    ...
    .then((_) => {
      ...
      console.log(
        `[OPENSEA] ${data.sale_type} - 3. nft id: ${nftData.id} updated owner ${nftData.owner} -> ${data.to}`
      );
  ...
  console.log(
    `[OPENSEA] ${data.sale_type} - 4. nft id: ${nftData.id} created SALE log id ${createdLog.id}`
  );
};

const buyOrderSaleProcessInElement = async ({ data, strapi, nftData }) => {
  console.log(
    `[OPENSEA] ${data.sale_type} - 2. buyOrderSaleProcessInElement Start`
  );
  /**
   * Wen DB 에 존재하는 NFT 임이 가정입니다. (Validation 완료)
...
module.exports = { openseaContractListener };
